...
def or_gate(train_func, forward_func, activation=identity, d_activation=dIdentity):
  print("\n=== OR ===")
...
def and_gate(train_func, forward_func, activation=identity, d_activation=dIdentity):
  print("\n=== AND ===")
  data: np.ndarray = np.array([[0, 0], [0, 1], [1, 0], [1, 1]], dtype=np.float32)
  target: np.ndarray = np.array([[0, 0], [0, 0], [0, 0], [1, 1]], dtype=np.float32)
  init_and_train(data, target, train_func, forward_func, activation, d_activation)

def xor_gate(train_func, forward_func, activation=identity, d_activation=dIdentity):
  print("\n=== XOR ===")
  data: np.ndarray = np.array([[0, 0], [0, 1], [1, 0], [1, 1]], dtype=np.float32)
